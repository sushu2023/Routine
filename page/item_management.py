...
def item_management_page():
    ...
    # 侧边栏：添加项目
    with st.sidebar.expander("添加分类项目"):
        with st.form("add_item_form"):
            item_id = st.text_input("分类项目 ID（需唯一）")  # 新增输入框
            name = st.text_input("分类项目名称")
            ...
            if submitted:
                try:
                    if not item_id:
                        st.toast("分类项目 ID 不能为空！", icon="❌")
                    elif not categories:
                        st.toast("请先添加分类！", icon="❌")
                    else:
                        category_id = category_options[category_name]
                        add_item(item_id=item_id, category_id=category_id, name=name, remark=remark)
                        st.toast("分类项目添加成功！", icon="✅")
                        st.rerun()  # 自动刷新页面
    ...
    if items and categories:
        with st.sidebar.expander("更新分类项目"):
            ...
            new_name = st.text_input("新分类项目名称")
    ...
    # 侧边栏：删除项目
    if items:
        with st.sidebar.expander("删除分类项目"):
